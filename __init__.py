...
def read_json_data(context, filepath, data_fields, data_array_name=None,
    ...
    '''
    Main function wrapped in this extension:
    Read json data from given filepath, create mesh object and
    store json data in mesh attributes
    '''
...
class ImportSpreadsheetData(bpy.types.Operator, ImportHelper):
    ...
    ImportHelper is a helper class, defines filename and invoke() function
    which calls the file selector.
    """
    # important since its how bpy.ops.import.spreadsheet is constructed
...
class RemoveDataFieldOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        operator.active_data_field_index = min(
            max(0, index - 1),
            len(operator.data_fields) - 1
        )
        return {'FINISHED'}
...
class RemoveJointOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        operator.active_joint_field_index = min(
            max(0, index - 1),
            len(operator.joint_names) - 1
        )
        return {'FINISHED'}
...
class SPREADSHEET_PT_json_options(bpy.types.Panel):
    ...
    def poll(cls, context):
        sfile = context.space_data
        operator = sfile.active_operator
        is_op = operator.bl_idname == "IMPORT_OT_spreadsheet"
        is_json = operator.filepath.lower().endswith('.json')
        return is_op and is_json
...
class SPREADSHEET_PT_joint_names(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        rows = 2
        joint_names_exist = bool(len(operator.joint_names) >= 1)
        if joint_names_exist:
            rows = 4
